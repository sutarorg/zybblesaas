...
**GitHub Actions on this branch** (`9c17b6b`, run `35750918159`): both jobs
green — *Frontend build, API typecheck, database tests* and **Worker build and
vet** (`go mod tidy`, `go vet ./...`, `go build ./...`). That run is the first
time the Go worker has ever been compiled, and it compiles clean. It also caught
two defects that local review had missed: progress counters left unused by an
earlier patch (a `vet` failure) and, after fixing that, a dropped closing brace
that made `watchSearchState` parse as a nested function. Every file in
`worker/` is now also parsed with a real Go grammar (tree-sitter-go) before
pushing.

## 20. What is *not* verified yet (do these before launch)

1. **The worker compiles, but has never been *run*.** `go mod tidy`, `go vet`
   and `go build` are green in CI on this branch, so it is syntactically and
   type-correct against the real Gosom/scrapemate APIs. Nothing has executed it
   yet: no database, no Chromium session, no Gemini call. Expect the first
   Railway deploy to surface runtime issues (page selectors, browser flags,
   memory) rather than compile issues. `go.sum` is generated by CI/Docker
   (`go.mod` pins gosom v1.18.1, scrapemate v1.4.0, pgx v5.7.2, uuid v1.6.0);
   commit a generated `go.sum` if you want reproducible builds.
...
4. **No database path has been executed outside the PGlite test harness.** The
   API smoke test exercises the real handlers with no `SUPABASE_*` values set, so
   every PostgREST/RPC call, every RLS policy against real auth claims, and the
   whole job-claim lifecycle are still unproven against a live project.
